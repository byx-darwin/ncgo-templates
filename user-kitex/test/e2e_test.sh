#!/usr/bin/env bash
# user-kitex 端到端测试：ncgo 加载模版 → 生成 → 静态断言 → sqlc → build → test
#
# 门槛：
#   - hermetic 基线：必跑（domain/application/infrastructure/oauth 单测不需要 DB/Redis）。
#   - postgres 变体：gated on pg_isready + POSTGRES_DSN（真实 DB 才跑 go test）。
# 任何被跳过的变体都会显式打印 `skipped: <原因>`（禁止静默跳过）。
set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
TPL_DIR="$REPO_ROOT/user-kitex"
MOD="example.com/user-e2e"
FAILS=0
...
assert_no_residual() { # $1=project dir
  ...
  if [ -d "$d/internal/handler/rulecenter" ] || grep -rn "api/ratelimit" "$d/kitex_gen" >/dev/null 2>&1; then
    fail "rule-center 预设泄漏（非 user-kitex 内容）in $d"
  fi
}
...
for tool in ncgo kitex protoc sqlc; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    skip "$tool 未安装（user-kitex e2e 需要 $tool）"
    exit 0
  fi
done
...
# --- 基线：hermetic（必跑）---
log "== hermetic 基线 =="
BASE_DIR="$(gen usere2e)"
assert_no_residual "$BASE_DIR"

# 结构性断言
log "== 结构性断言 =="
PROTO="$BASE_DIR/idl/user.proto"
SCHEMA="$BASE_DIR/internal/db/schema/000001_user.sql"

if grep -q "service UserService" "$PROTO"; then log "proto has UserService"; else fail "proto missing UserService"; fi
if grep -q "rpc OAuthCallback" "$PROTO"; then log "proto has OAuthCallback"; else fail "proto missing OAuthCallback"; fi
if grep -q "rpc ListUsers" "$PROTO"; then log "proto has admin ListUsers"; else fail "proto missing admin ListUsers"; fi

if grep -q "CREATE TABLE users" "$SCHEMA" && grep -q "CREATE TABLE user_identities" "$SCHEMA"; then
  log "schema has users + user_identities"
else
...
if [ -f "$BASE_DIR/internal/pkg/oauth/wechat.go" ] && [ -f "$BASE_DIR/internal/pkg/oauth/alipay.go" ] && \
  ...
  log "all five OAuth provider adapters generated"
else
  fail "one or more OAuth provider adapter files missing"
fi
