...
per-service router/handler files (`internal/router/pb/{service}.go`,
`internal/handler/pb/{service}_service.go`) are generated by ncgo's
`loop_service` overlay from `idl/app/{service}.proto`; without that fix the
per-service `Register` is not generated and `go build` fails with
`undefined: pb.Register`.
